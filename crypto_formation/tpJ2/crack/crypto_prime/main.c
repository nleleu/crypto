...
#define CHUNK_FILE_NAME "prime_chunk"
#define CHUNK_FILE_NAME_ID "prime_chunk_%u"
#define MAX_CHUNK_ID 100
//TODO : dump dans plein de fichiers.
//Choix du fichier de départ : contient un nb prime < sqtr(n)
...
static chunk_file_t chunk_file_array[MAX_CHUNK_ID];
static unsigned int chunk_file_array_nr;
static unsigned int chunk_file_array_pos;
...
void dump_prime_numbers()
{
    ...
    while (current)
    {
        ...
        char filename[255];
        snprintf(filename, 255, CHUNK_FILE_NAME_ID, chunk_file_array_pos);
        chunk_file_array[chunk_file_array_pos].id = chunk_file_array_pos;
    ...
    for (i = 0; i < prime_number_array->prime_numbers_nr; i++)
    {
        if (prime_number_array->array[i].simple ==0 )
        {
            if (i == 0)
            {   
                chunk_file_array[chunk_file_array_pos].start.simple = 0;
                mpz_init(chunk_file_array[chunk_file_array_pos].start.mpz);
                mpz_set (chunk_file_array[chunk_file_array_pos].start.mpz, prime_number_array->array[i].mpz);
            }
                chunk_file_array[chunk_file_array_pos].stop.simple = 0;
                mpz_init(chunk_file_array[chunk_file_array_pos].stop.mpz);
                mpz_set (chunk_file_array[chunk_file_array_pos].stop.mpz, prime_number_array->array[i].mpz);
        ...
        else
        {
            if (i == 0)
            {
                chunk_file_array[chunk_file_array_pos].start.simple = prime_number_array->array[i].simple;
            }

                chunk_file_array[chunk_file_array_pos].stop.simple = prime_number_array->array[i].simple;
            fprintf(fp, "%lu", prime_number_array->array[i].simple);            
        }
        fprintf(fp, "\n"); 
    }
    printf("save chunk id %u start : ");
    if (chunk_file_array[chunk_file_array_pos].start.simple)
    {
        printf("%u", chunk_file_array[chunk_file_array_pos].start.simple);
    }
    else
        mpz_out_str (stdin, 10, chunk_file_array[chunk_file_array_pos].start.mpz);
    printf(", stop : ");
    if (chunk_file_array[chunk_file_array_pos].stop.simple)
    {
        printf("%u", chunk_file_array[chunk_file_array_pos].stop.simple);
    }
    else
        mpz_out_str (stdin, 10, chunk_file_array[chunk_file_array_pos].stop.mpz);
    printf("\n");
    fclose(fp);
    chunk_file_array_pos++;
...
void clean_chunks()
{
    ...
    for (i = 0; i < prime_number_array->prime_numbers_nr; i++)
    {
        if (prime_number_array->array[i].simple ==0 )
        {
            mpz_clear(prime_number_array->array[i].mpz);
        }
    }
    chunk_t* tmp = current;
    current = current->next;
    free(prime_number_array->array);
